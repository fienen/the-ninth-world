...
class Publication_Meta_Box {
	...
	public function add_meta_boxes() {
		foreach ( $this->screens as $screen ) {
			add_meta_box(
				'publication-details',
				__( 'Publication Details', 'the-ninth-world' ),
				array( $this, 'add_meta_box_callback' ),
				$screen,
				'normal',
				'high'
	 ...
	 */
	public function add_meta_box_callback( $post ) {
		wp_nonce_field( 'publication_details_data', 'publication_details_nonce' );
		$this->generate_fields( $post );
	}
	...
	public function generate_fields( $post ) {
		...
		foreach ( $this->fields as $field ) {
			$label = '<label for="' . $field['id'] . '">' . $field['label'] . '</label>';
			$db_value = get_post_meta( $post->ID, 'publication_details_' . $field['id'], true );
			switch ( $field['type'] ) {
				case 'select':
					$input = sprintf(
						'<select id="%s" name="%s">',
	 ...
	 */
	public function save_post( $post_id ) {
		if ( ! isset( $_POST['publication_details_nonce'] ) )
			return $post_id;

		$nonce = $_POST['publication_details_nonce'];
		if ( !wp_verify_nonce( $nonce, 'publication_details_data' ) )
			return $post_id;
		...
		foreach ( $this->fields as $field ) {
			if ( isset( $_POST[ $field['id'] ] ) ) {
				...
				update_post_meta( $post_id, 'publication_details_' . $field['id'], $_POST[ $field['id'] ] );
			} else if ( $field['type'] === 'checkbox' ) {
				update_post_meta( $post_id, 'publication_details_' . $field['id'], '0' );
			}
		}
	}
}
new Publication_Meta_Box;
